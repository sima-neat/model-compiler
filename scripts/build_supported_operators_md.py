#!/usr/bin/env python3
"""Generate docs/guides/supported_operators.md by merging two operator sources.
...
OUT = REPO_ROOT / "docs" / "guides" / "supported_operators.md"
...
def main() -> int:
    ...
    lines += [
        ...
        "# Supported Operators",
        "",
        "Operators the SiMa.ai MLA compiler supports and the precision schemes each "
        "one runs in. **INT8** runs on the MLA; **BF16** is available on Modalix "
        "(developer preview). **5D** marks operators that accept 5D (N, D, H, W, C) "
        "tensors. **Opset** is the ONNX opset version.",
        "",
