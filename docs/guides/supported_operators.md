...
Operators the SiMa.ai MLA compiler supports and the precision schemes each one runs in. **INT8** runs on the MLA; **BF16** is available on Modalix (developer preview). **5D** marks operators that accept 5D (N, D, H, W, C) tensors. **Opset** is the ONNX opset version.
