...
The SiMa.ai compilation toolchain imports **ONNX** models and prepares the operators that run on the Machine Learning Accelerator (MLA). Use this page to check whether a model's operators are supported before preparing it for Modalix.

## Supported operators

The table lists the operators the MLA compiler supports and the precision schemes each one runs in. **INT8** runs on the MLA; **BF16** is available on Modalix (developer preview). **5D** marks operators that accept 5D (N, D, H, W, C) tensors. **Opset** is the ONNX opset version.
